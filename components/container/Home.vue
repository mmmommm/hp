  ...
  <section id='home' class='md:h-screen md:flex md:items-center bg-pink-100'>
    <div class='md:w-1/2'>
      <div class='hidden md:block flex md:ml-48 md:mt-32 lg:mt-12'>
        <img src='image/Bicycle1.png' alt='鹿児島で撮った自転車二台の映った画像' class='h-1/2 homeImage1'>
        <img src='image/Bicycle2.png' alt='父母ヶ浜で撮った自転車と一緒に撮った画像' class='h-1/2 homeImage2'>
      </div>
      <div class='md:hidden'>
        <img src='image/smimage.png' data-src='~assets/image/smimage.png' alt='父母ヶ浜で撮った自転車と一緒に撮った画像' class='smimage'>
      </div>
    </div>
...
export default class Home extends Vue {
  ...
  mounted() {
    ...
    gsap.fromTo('.welcome', { duration: 0.5, opacity: 0, y: 50 },{ delay: 0, opacity: 1, y: 150 })
    gsap.fromTo('.to', { duration: 0.5, opacity: 0, y: 300 }, { delay: 0.5,opacity: 1, y: 200 })
    gsap.to('.welcome', { duration: 0.5, delay: 1.25 , opacity: 0, y: 50})
    gsap.to('.to', { duration: 0.5, delay: 1.5 , opacity: 0, y: 250})
    document.querySelectorAll('.title').forEach((title, i) => {
      gsap.from(title, { duration:0.4, delay: 1.25 + i * 0.1, ease: 'power1.out', x: 50, y: -50, opacity: 0 })
    })
  }
}
</script>
